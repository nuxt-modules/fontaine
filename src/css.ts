...
export const generateFontFace = (metrics: Font, options: { name: string, fallbacks: string[], [key: string]: any }) => {
  ...
  const declaration = {
    ...
    'ascent-override': toPercentage(metrics.ascent / (metrics.unitsPerEm * sizeAdjust)),
    'descent-override': toPercentage(Math.abs(metrics.descent / (metrics.unitsPerEm * sizeAdjust))),
    'line-gap-override': toPercentage(metrics.lineGap / (metrics.unitsPerEm * sizeAdjust)),
    ...properties,
  }
